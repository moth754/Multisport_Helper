#!/usr/bin/env bash
# Multisport Helper installer for Raspberry Pi OS, Debian and Ubuntu.
# Safe to re-run at any time: it updates the Python packages and the service.
#
#   ./install.sh               install, and start automatically at boot on port 8081
#   ./install.sh --port 8091   use a different port
#   ./install.sh --no-service  only set up Python; start it yourself with ./run.sh
#   ./install.sh --uninstall   remove the service (keeps data/)
set -euo pipefail

APP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SERVICE_NAME="multisport"
...
if [ "$UNINSTALL" -eq 1 ]; then
    ...
    echo "Done. Your settings are still in $APP_DIR/data."
    exit 0
fi
...
echo "Settings and the page password are kept in $APP_DIR/data, which is never added to git."
...
sed -e "s|@APP_DIR@|$APP_DIR|g" -e "s|@USER@|$RUN_USER|g" -e "s|@GROUP@|$RUN_GROUP|g" \
    -e "s|@PORT@|$PORT|g" \
    "$APP_DIR/deploy/multisport.service" > "$tmp"
...
echo "The Multisport Helper is running and will start automatically after a reboot."
